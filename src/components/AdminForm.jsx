...
export const AdminForm = ({ initialService, onSubmit, onCancel }) => {
  ...
  const handleSubmit = (event) => {
    ...
    if (!initialService) setFormData(EMPTY_SERVICE);
  };
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-5">
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        {onCancel && (
          <button type="button" onClick={onCancel} className="sm:w-1/3 bg-slate-100 hover:bg-slate-200 text-slate-700 py-3.5 rounded-xl font-bold transition flex items-center justify-center gap-2">
            <Icon name="X" size={18} /> Cancelar
          </button>
        )}
        <button type="submit" className="flex-1 bg-cyan-600 hover:bg-cyan-700 text-white py-3.5 rounded-xl font-bold text-lg transition shadow-md flex items-center justify-center gap-2">
